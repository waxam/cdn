const gulp=require("gulp"),fs=require("fs"),path=require("path"),_=require("lodash"),rename=require("gulp-rename"),replace=require("gulp-replace"),stripCssComments=require("strip-css-comments"),decomment=require("decomment"),sourcemaps=require("gulp-sourcemaps"),packageJson=require("./package.json");gulp.task("merge",()=>{return gulp.src("./src/"+packageJson.wcfactory.elementName+".js").pipe(replace(/\/\* REQUIRED FOR TOOLING DO NOT TOUCH \*\//g,()=>{let d=fs.readFileSync(path.join("./",packageJson.wcfactory.files.html)).toString().trim();d=decomment(d);let e="";if(packageJson.wcfactory.useHAX){let h=fs.readFileSync(path.join("./",packageJson.wcfactory.files.hax));e=`
  // haxProperty definition
  static get haxProperties() {
    return ${h};
  }`}let f="{}";f=fs.readFileSync(path.join("./",packageJson.wcfactory.files.properties));let g="<style>";if(packageJson.wcfactory.useSass&&packageJson.wcfactory.files.scss){const h=require("node-sass");g+=h.renderSync({file:path.join("./",packageJson.wcfactory.files.scss)}).css}else packageJson.wcfactory.files.css&&(g+=fs.readFileSync(path.join("./",packageJson.wcfactory.files.css)));return g+="</style>",g=stripCssComments(g).trim(),`
  // render function
  static get template() {
    return html\`
${g}
